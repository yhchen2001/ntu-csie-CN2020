...
func ChatAsAnon(conn net.Conn) {
	...
	// starts chatting
	asAnonDisplayHistory(conn)

	asAnonStartChat(conn)
}

func asAnonStartChat(conn net.Conn){
	...
	exit := false
	finish := false
	go asAnonMonitor(conn, &exit, &finish)

	defer func(){
		log.Println("waiting for asAnonMonitor to finish")
		for !finish{
		}
	}()
...
func asAnonMonitor(conn net.Conn, exit * bool, finish * bool){
	log.Println("start asAnonMonitoring")
	for !(*exit) {
		msg,err := transfer.RecvMsg(conn)
		if err != nil{
			log.Println("asAnonMonitor read error, break :", err)
			break
		}
		fmt.Print(msg)
	}
	log.Println("finish asAnonMonitoring")
	*finish = true
}

func asAnonDisplayHistory(conn net.Conn) {
